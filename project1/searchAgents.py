...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if state[0] in self.corners: #if the node we're at is a corner then we can add it to the list if needed
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successor = ((((nextx, nexty), visited_corners)), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    #we can just bank on the manhattan distance to each corner - that gives us a value for each corner and then we could add them 
    #           together
    ...
    if len(visited_corners) ==4:
        return 0
    #we need to find what corners we have left to visit
    corners_left = list()
...
def foodHeuristic(state, problem):
    ...
    for i in range(0, len(foodGrid)):
        ...
        maze_distance = mazeDistance(current_node, foodGrid[i], problem.startingGameState)
        heurs += [maze_distance]
    #print heurs
    return max(heurs)
